#%% 选取162个特征(9*18 9h 18feature)，进行预测第10小时pm2.5值
#导入模块
import numpy as np
import pandas as pd
...
#--------------- 模型建立---线性回归 ---------------
#1.归一化
#2.利用前9小时的PM2.5 预测第十个小时的PM2.5
#标准差归一化
pm = new_train_data.values
pm_mean = (pm.mean(axis=0)) #按每列求平均，每个指标的平均
pm_theta = (pm.var(axis=0)**0.5)
pm = (pm-pm_mean)/pm_theta

##最大最小值归一
#pm = new_train_data.values
#pm_max = pm.max(axis=0)
#pm_min = pm.min(axis=0)
#pm = (pm-pm_min)/(pm_max-pm_min)

xx_len=9*18
w = np.random.rand(1,xx_len+1) #9个小时 19个fratue
lr = 0.005
m = len(label)

import time 
start=time.time()
for i in range(200): #训练所有数据100次
    loss=0
    i+=1
    gradient=0
    gra_2 = 0
    ...
    for j in range(m):
        ...
        error = label[j] - y_pre
        loss += (error)**2
        gradient += -2*error*x #对w一次的更新163*1
        ''' #正则化代码
        reg_rate = 1 #正则项系数
        ww = w.copy()
        ww = ww[1:]
        ww = np.insert(ww,0,0)
        gradient += reg_rate*ww.T
        '''
        gra_2 += (2*error*x)**2
    loss = loss/(2*m) #平均下
    gra_2 = gra_2/m
...
#a = y_pre- y_real.values
#plt.plot(a) #查看插值，是否有特别大的，导致平方后变大。
